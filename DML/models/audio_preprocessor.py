...
def normalize_batch(x, seq_len, normalize_type):
    ...
    if normalize_type == "per_feature":
        ...
        if torch.cuda.is_available() and not torch.cuda.is_current_stream_capturing() and torch.any(seq_len == 1).item():
            raise ValueError(
        ...
        x_std = torch.sqrt(torch.sum(torch.where(valid_mask.unsqueeze(1), x - x_mean.unsqueeze(2), 0.0) ** 2, axis=2) / (x_mean_denominator.unsqueeze(1) - 1.0))
...
class FilterbankFeatures(nn.Module):
    ...
    def __init__(
        ...
        filterbanks = torch.tensor(
            librosa.filters.mel(sr=sample_rate, n_fft=self.n_fft, n_mels=nfilt, fmin=lowfreq, fmax=highfreq, norm=mel_norm),
            dtype=torch.float,
        ).unsqueeze(0)
    ...
    def log_zero_guard_value_fn(self, x):
        if isinstance(self.log_zero_guard_value, str):
            ...
            raise ValueError(f"{self} received {self.log_zero_guard_value} for the log_zero_guard_type parameter. It must be either a number, 'tiny', or 'eps'")
    ...
    def forward(self, x, seq_len, linear_spec=False):
        ...
        if self.stft_pad_amount is not None:
            x = torch.nn.functional.pad(x.unsqueeze(1), (self.stft_pad_amount, self.stft_pad_amount), "constant").squeeze(1)
...
class AudioPreprocessor(nn.Module, ABC):
    ...
    def __init__(self, win_length=400, hop_length=160):
        super().__init__()
